...
import { getEventsAfter } from './get-events-after';

describe('Function `getEventsAfter`', () => {
  let knex: Knex;
  ...
  it('returns all events for the stream before the version', async () => {
    ...
    const output = await getEventsAfter(knex, STREAM_ID, 1);
  ...
  it('returns all events for the stream before the timestamp', async () => {
    ...
    const output = await getEventsAfter(knex, STREAM_ID, mark);
